...
var vm = new Vue({
    ...
    methods:{
        ...
        editar: function (cat) {
            ...
            $('#categoria_blog_nome').val(cat.categoria_blog_nome);
            $('#categoria_blog_id').val(cat.categoria_blog_id);
            $('#categoria_blog_exibir_home').val(cat.categoria_blog_exibir_home);
        },
        limpar: function () {
            $('.categoria-acao').html('Incluir');
            $('#categoria_blog_nome').val('');
            $('#categoria_blog_id').val('');
        },
        remover: function (dt) {
            vm.$data.rm = dt.categoria_blog_id;
            new bootstrap.Modal(document.getElementById('modal-remove')).show();
        },
        remove: function (id) {
            var url_remove = baseUri + '/BlogAdmin/remover_categoria/';
...
$(document).ready(function () {
    setTimeout(function () {
        table.on('row-reorder', function (e, diff, edit,) {
            var url = baseUri + '/blogAdmin/ordenar/';
            if (diff.length > 0) {
