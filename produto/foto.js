...
var vm = new Vue({
    ...
    methods: {
        listar: function () {
            var url = baseUri + '/galeria/lista_img_galeria/id/' + produto_id;
            var _this = this;
            $.getJSON(url, function (dados) {console.log(dados);
        ...
        add_class_remove: function (item) {
            ...
            var ele = $('#img-galeria-id-' + item.foto_id);
        ...
        remover: function () {
            ...
            $.post(url, { produto_id: produto_id, data: _this.$data.data_remove }).then(function (rs) {
                console.log(rs);
                if (parseInt(rs) == 0) {
        ...
        remove_all: function () {
            ...
            $.post(url, { produto_id: produto_id }).then(function (rs) {
                console.log(rs);
                if (parseInt(rs) == 0) {
                    alert_success('Fotos apagadas com sucesso!');
                    this.selected = false;
...
$(document).ready(function () {
    ...
    var sortable = Sortable.create(el, {
        ...
        onChange: function (evt) {
            ...
            $.post(url, { produto_id: produto_id, data: data }).then(function (rs) {
                if (parseInt(rs) == 0) {
