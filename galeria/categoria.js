...
var vm = new Vue({
    ...
    methods:{
        ...
        editar: function (cat) {
            ...
            $('#categoria_galeria_nome').val(cat.categoria_galeria_nome);
            $('#categoria_galeria_id').val(cat.categoria_galeria_id);
        },
        limpar: function () {
            $('.categoria-acao').html('Incluir');
            $('#categoria_galeria_nome').val('');
            $('#categoria_galeria_id').val('');
        },
        remover: function (dt) {
            vm.$data.rm = dt.categoria_galeria_id;
            new bootstrap.Modal(document.getElementById('modal-remove')).show();
        },
        remove: function (id) {
            var url_remove = baseUri + '/galeria/remover_categoria/';
...
$(document).ready(function () {
    setTimeout(function () {
        table.on('row-reorder', function (e, diff, edit,) {
            var url = baseUri + '/galeria/ordenar_cat/';
            if (diff.length > 0) {
