var vm = new Vue({
    ...
    methods: {
        ...
        remove: function (id) {
            var url_remove = baseUri + '/receber/remover/';
...
$('#modal-status #status').on('change', function () {
    ...
    if (st == 3) {
        $('#modal-status #dv-datarecbo').removeClass('d-none');
        if ($('#modal-status #datarecbo').val() == "") {
            var today = new Date().toLocaleDateString('pt-BR');
            var time = new Date().getHours() + ":" + new Date().getMinutes();
            $('#modal-status #datarecbo').val(today);
        }
    } else {
        $('#modal-status #dv-datarecbo').addClass('d-none');
        $('#modal-status #datarecbo').val('');
    }
});
...
$('#status').on('change', function () {
    ...
    if ($(this).val() != 3) {
        ...
        if (olddt != '') {
            ...
            var bc = $('#datarecbo').css('border-color');
            $('#datarecbo').css('border-color', 'red');
            setTimeout(function () {
                $('#datarecbo').css('border-color', bc);
            }, 2000);
        }
        $('#datarecbo').val(dt);
    }
